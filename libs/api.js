...
const { apiFarzain, apiItech, apiZeks, apiLol, apiGenius, apiOcr } = process.env

const quote = () => new Promise((resolve, reject) => {
  const slash = sample(['quotes', 'quotes2', 'quotes3'])
  get(`https://api.i-tech.id/tools/${slash}?key=${apiItech}`)
    .then((res) => {
      let text = ''
      const hasAuthorProperty = Object.prototype.hasOwnProperty.call(res.data, 'author')
...
const lyric = (query) => new Promise((resolve, reject) => {
  get(`https://api.genius.com/search?q=${encodeURIComponent(query)}&access_token=${apiGenius}`)
    .then(res => {
      if (res.data.meta.status !== 200) resolve(`${res.data.meta.status} ${res.data.meta.message}`)
